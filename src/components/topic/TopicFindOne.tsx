...
export const TopicFindOne = () => {
  ...
  const {
    ...
    error,
    fetchData,
  } = useGet<Topic>(`/api/topics/${id}`);
  const navigate = useNavigate();

  const handleRemove = () => {
    const confirmed = window.confirm(
      `¿Desea eliminar el topic: ${topic?.description}?`
    );
    if (confirmed) {
    ...
    <div>
      ...
      <p>
        ...
        <button className="edit-button">
          <Link
            to={`/topic/update/${topic?.id}?description=${topic?.description}`}
          >
            Edit
          </Link>
        </button>
...
export const TopicGetOne: React.FC<TopicGetOneProps> = ({ id }) => {
  ...
  return <Container>{topic?.description}</Container>;
};
